...
const Listings: React.FC = () => {
  ...
  const filteredProperties = useMemo(() => {
    return properties.filter(p => {
      if (favoritesOnly && currentUser) {
        if (!currentUser.favorites?.includes(p.id)) return false;
      }
  ...
  }, [properties, filterType, sortOrder, searchQuery, priceRange, favoritesOnly, currentUser]);
            ...
            <div className="flex flex-wrap gap-4">
              ...
              {currentUser && (
                <button
                  ...
                  className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest border ${favoritesOnly ? 'bg-black text-white border-black' : 'bg-white text-black border-slate-300'} transition-all`}
                  title="Show only favorites"
                >
                  <span className="inline-flex items-center gap-2">
                    <Icon name="heart" variant={favoritesOnly ? 'solid' : 'regular'} />
                    Favorites
                  </span>
                </button>
              )}
