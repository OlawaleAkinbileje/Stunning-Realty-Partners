          ...
          <div className="hidden lg:flex items-center space-x-6">
            {currentUser ? (
              <div className="flex items-center space-x-4">
                ...
                <button onClick={onLogout} className="text-[10px] uppercase tracking-widest font-bold text-slate-400 hover:text-red-600 transition-colors">
                  Logout
                </button>
              </div>
            ) : (
      ...
      {isMenuOpen && (
        <div className="lg:hidden glass border-b border-slate-200 animate-in slide-in-from-top duration-300">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            {currentUser ? (
              <>
                <Link to="/profile" onClick={() => setIsMenuOpen(false)} className="block px-3 py-3 text-sm font-bold uppercase tracking-widest text-slate-600 hover:bg-slate-50">Profile</Link>
                <button onClick={() => { onLogout(); setIsMenuOpen(false); }} className="block w-full text-left px-3 py-3 text-sm font-bold uppercase tracking-widest text-red-600 hover:bg-slate-50">Logout</button>
              </>
            ) : (
              <Link to="/auth" onClick={() => setIsMenuOpen(false)} className="block px-3 py-3 text-sm font-bold uppercase tracking-widest text-black hover:bg-slate-50">Join Network</Link>
            )}
          </div>
        </div>
      )}
